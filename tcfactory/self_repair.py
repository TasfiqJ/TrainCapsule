...
def build_self_repair_task(*, reason: str, attempt: int, task_id: str) -> TaskPacket:
    ...
    gates = [
        Gate(
            name="no-paid-usage",
            command="bash scripts/gates/no_paid_usage.sh",
            stages=[RoleName.FACTORY_REPAIR],
        ),
        Gate(
            name="secret-scan",
        ...
        Gate(
            name="self-repair-scope",
            command="bash scripts/gates/self_repair_scope.sh",
            stages=[RoleName.FACTORY_REPAIR],
        ),
    ]
    common_criteria = [
