...
class MilestoneExitCriterionContract(V3Model):
    ...
    required_external_evidence_types: list[EvidenceType] = Field(
        default_factory=list[EvidenceType]
    )
...
class WorkItemEvidenceContract(V3Model):
    ...
    allowed_external_evidence_types: list[EvidenceType] = Field(
        default_factory=list[EvidenceType]
    )
...
class MilestoneEvidenceContract(V3Model):
    ...
    machine_policy_required: bool
    allow_unrelated_future_lanes_while_external_wait: list[str] = Field(
...
class CompletionEvidenceObservation(V3Model):
    ...
    external_type_counts: dict[EvidenceType, int] = Field(
        default_factory=dict[EvidenceType, int]
    )
...
def evaluate_work_item_evidence_contract(
    ...
    for authority in contract.required_authorities:
        if authority not in observed_authorities:
            failures.append(
                f"{contract.work_item_id} lacks {authority.value} authority"
            )
    for semantic in contract.required_semantics:
        ...
        if observation.semantic_counts.get(semantic, 0) < minimum:
            failures.append(
                f"{contract.work_item_id} lacks {minimum} {semantic.value} evidence"
            )
...
def evaluate_milestone_exit_criteria(
    ...
    *,
    ...
    for criterion in contract.exit_criteria:
        ...
        if missing_items:
            failures.append(
                f"{criterion.criterion_id} lacks work-item evidence: "
        ...
        for evidence_type in criterion.required_external_evidence_types:
            if external_type_counts.get(evidence_type, 0) < 1:
                failures.append(
                    f"{criterion.criterion_id} lacks {evidence_type.value} evidence"
                )
        ...
        if criterion.machine_policy_required and not machine_policy_available:
            failures.append(
                f"{criterion.criterion_id} lacks machine-policy authorization"
            )
    return failures
...
def load_completion_evidence_policy(repo_root: Path) -> CompletionEvidencePolicy:
    ...
    expected_work_items = "sha256:" + sha256_file(
        repo_root / "factory/roadmap/work_items.yaml"
    )
    expected_milestones = "sha256:" + sha256_file(
        repo_root / "factory/roadmap/milestones.yaml"
    )
    if (
