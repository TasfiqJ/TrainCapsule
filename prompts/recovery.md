...
Repair only the causal defect inside authorized paths. Do not alter product authority, value thresholds, machine policy, private evidence, roadmap state, or unrelated product behavior. Never use force-push, bypass controller-owned main promotion, broad history rewrite, or zero-as-unbounded retry semantics.

Re-run the exact affected gates and independent oracle. Keep infrastructure errors distinct from product FAIL and do not claim full recovery from partial state checks. On missing external truth use WAITING_EXTERNAL; on missing machine authority use BLOCKED_POLICY. When the retry or repeated-fingerprint limit is exhausted, emit HARD_STUCK and stop.
