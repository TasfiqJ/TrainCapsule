...
def test_all_native_v31_contracts_accept_coherent_records() -> None:
    ...
    benchmark = NativeSubstituteBenchmarkV31(
        ...
        source_freshness_receipts=[DIGEST],
        ...
        native_inputs=[DIGEST],
        native_outputs=[DIGEST],
        native_findings=[DIGEST],
        native_operational_decision="Native evidence did not resolve the incident contract.",
        traincapsule_incremental_capability="Qualification changes the bounded action.",
        traincapsule_outputs=[DIGEST_B],
        traincapsule_operational_decision="Run the bounded candidate qualification.",
        native_evidence_digests=[DIGEST],
        candidate_evidence_digests=[DIGEST_B],
        incident_derived_contract_digests=[DIGEST],
        ...
        cost_time_resource_comparison=CostTimeResourceComparisonV31(
            schema_version="3.1",
            native_cost=100.0,
            traincapsule_cost=25.0,
            native_minutes=30.0,
            traincapsule_minutes=10.0,
            native_resources=["native-tool"],
            traincapsule_resources=["traincapsule"],
        ),
        reproducibility=BenchmarkReproducibilityV31(
            schema_version="3.1",
            repetitions=2,
            matching_decision_count=2,
            command_digests=[DIGEST],
            environment_digest=DIGEST_B,
        ),
        limitations=["Controlled evidence is not external value evidence."],
        truth_state=EpistemicState.CONTROLLED,
        raw_artifact_hashes=[DIGEST, DIGEST_B],
...
def test_fingerprint_and_native_value_states_cannot_be_laundered() -> None:
    ...
    benchmark = {
        ...
        "approvedAgentAssistanceBaseline": [],
        "nativeTool": "PyTorch Flight Recorder",
        "nativeToolVersion": "2.5.1",
        "nativeInputs": [DIGEST],
        "nativeOutputs": [DIGEST],
        "nativeFindings": [DIGEST],
        "nativeOperationalDecision": "Native evidence did not resolve the contract.",
        "traincapsuleIncrementalCapability": "Qualification changes the action.",
        "traincapsuleOutputs": [DIGEST_B],
        "traincapsuleOperationalDecision": "Run qualification.",
        "nativeEvidenceDigests": [DIGEST],
        "candidateEvidenceDigests": [DIGEST_B],
        "incidentDerivedContractDigests": [DIGEST],
        "independentOracleIds": ["ORACLE:NATIVE:001"],
        "oracleIdentity": "ORACLE:NATIVE:001",
        "issuerIdentity": "VERIFIER:LOCAL:001",
        "nativeEffortMinutes": 30.0,
        "candidateEffortMinutes": 10.0,
        "costTimeResourceComparison": {
            "schemaVersion": "3.1",
            "nativeCost": 100.0,
            "traincapsuleCost": 25.0,
            "nativeMinutes": 30.0,
            "traincapsuleMinutes": 10.0,
            "nativeResources": ["native-tool"],
            "traincapsuleResources": ["traincapsule"],
        },
        "reproducibility": {
            "schemaVersion": "3.1",
            "repetitions": 2,
            "matchingDecisionCount": 2,
            "commandDigests": [DIGEST],
            "environmentDigest": DIGEST_B,
        },
        "limitations": ["Controlled-only."],
        "truthState": EpistemicState.CONTROLLED,
        "rawArtifactHashes": [DIGEST, DIGEST_B],
