# 14 — Claude Code Master Build Prompt V3.1-ZH
| Field | Value |
|---|---|
| Logical ID | `TC.V3_1_ZH.MASTER_BUILD_PROMPT` |
| Generation | `traincapsule-v3.1-zh-2026-08-12` |
| Authority class | `normative_build` |
| Derived from | `TC.V3.MASTER_BUILD_PROMPT` |
...
1. signed independent machine-policy authorization for its exact scope;
...
## Independent machine authority

No external or commercial release is approved solely by you or another AI session.

When independent machine-policy authorization is required:

1. prepare an machine-policy request;
2. bind it to the exact candidate SHA and artifact digests;
3. list evidence, limitations, and required verifier-policy and private-oracle coverage;
4. stop in `BLOCKED_POLICY`;
5. do not create the signed machine-policy receipt yourself.
...
8. stop if the packet asks for external evidence, customer action, or independent machine-policy authorization that you cannot provide.
...
- required `UNKNOWN` blocks unconditional authorization;
...
- do not change product authority, value thresholds, machine-policy authorization policy, or private evidence;
- finite retries only;
- no zero/unlimited semantics;
- no direct main promotion;
- completion evaluation roles propose work but cannot mutate the roadmap;
...
- independent machine-policy authorization is required;
