# 00 — Executive Build Decision V3.1-ZH
| Field | Value |
|---|---|
| Logical ID | `TC.V3_1_ZH.EXECUTIVE_DECISION` |
| Generation | `traincapsule-v3.1-zh-2026-08-12` |
| Authority class | `root_policy` |
| Derived from | `TC.V3.EXECUTIVE_DECISION` |
...
- **External release authority:** independent machine-policy authorization required
...
## 00.10 Independent machine authority
...
Before first external use and before every new commercial incident pack, a independent machine-policy verifier must approve:
...
- incapable of commercial release without independent machine-policy authorization.
...
→ MACHINE_POLICY_ATTESTATION / DEFER / SUPERSEDE / WEDGE_REJECT
```
...
- paid pilot offer;
- policy-controlled outreach and attributable external sales actions.
...
- V3.1-ZH authority installed;
- finite limits;
- lanes and task types;
- no current critical-path T002;
- PR/machine-policy release modes;
- old bundle archived;
- tests pass.
...
- independent machine-policy trust authorization;
- security package;
- operator guide;
...
- machine-policy release authorization current;
...
- third same-family case without trust-core rewrite;
- one independent external operator executes and understands the result;
...
The factory may recommend a disposition. Independent signed machine policy records the final bounded wedge decision.
