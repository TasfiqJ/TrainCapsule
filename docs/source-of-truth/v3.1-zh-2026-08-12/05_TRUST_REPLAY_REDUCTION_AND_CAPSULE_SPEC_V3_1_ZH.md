# 05 — Trust, Replay, Reduction, Recovery, and Capsule Specification V3.1-ZH
| Field | Value |
|---|---|
| Logical ID | `TC.V3_1_ZH.TRUST_SPECIFICATION` |
| Generation | `traincapsule-v3.1-zh-2026-08-12` |
| Authority class | `normative_trust` |
| Derived from | `TC.V3.TRUST_SPECIFICATION` |
...
- Any required `FAIL` blocks authorization unless an authorized independent machine policy explicitly defines another outcome.
- Any required `UNKNOWN` prevents unconditional authorization.
...
|---|---|---|
| Trigger reproduces | Trigger absent and required recovery properties pass | potential authorization within envelope |
...
| Baseline inconclusive | Candidate conclusive | no direct comparative authorization; more evidence |
...
- independent machine-policy authorization expires.
...
## 05.20 Independent machine authority
...
Before first external use and before each new commercial incident pack, a independent machine-policy verifier must approve:
...
### Qualification of independent verifier policy and private oracles

The machine-policy request records why the verifier policy and private oracles are sufficient, such as:
...
One person need not cover all domains. Multiple scoped signed machine-policy receipts are preferred.
...
- compare documents;
- prepare machine-policy requests;
...
- convert `UNKNOWN` into authorization.
...
10. independent machine-policy authorization is signed;
...
- independent machine-policy authorization is required and absent;
- contract is expired;
